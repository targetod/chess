...
{
    // cancels any of the asunc operation
    // service stop ??
/*
    deadline_timer timer(*io_ptr);
    timer.expires_from_now(boost::posix_time::seconds(60));
    timer.async_wait([&io_ptr](const boost::system::error_code& ) {
        io_ptr->stop();
    });
*/
}
...
void AsyncAiPlayer::prepare(const ChessBoard &board, ReadyHandler handler)
{
    //  do something ...
...
void AsyncAiPlayer::getMove(const ChessBoard & orig_board, MoveReadyHandler handler)
                       //Move & move, AdvancedMoveData *move_data)
{
...
void AsyncAiPlayer::showMove(const ChessBoard &board,const Move &move, ReadyHandler handler)
{
    //....
    handler();
}

int AsyncAiPlayer::evalAlphaBeta(ChessBoard & board, const EvaluationInformation * info) const
{
#ifdef TRACE
    list<Move> chain;
...
int AsyncAiPlayer::evaluateBoard(const ChessBoard & board) const
